...
Tree.shiftUp = function(tree) {
    ...
    var tmp = selected.parent.childNodes[childNum];
    selected.parent.childNodes[childNum] = selected.parent.childNodes[childNum - 1]
    selected.parent.childNodes[childNum - 1] = tmp;
};

Tree.shiftDown = function(tree) {
    var selected = Tree.findSelected(tree);
    var childNum = Tree.findChildNum(selected);
    if (childNum == selected.parent.childNodes.length - 1) {
        return;
    }
    if (selected.parent.childNodes.length <= 1) {
        return;
